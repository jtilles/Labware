...
int main(void){  
  ...
  while(1){
		GPIO_PORTF_DATA_R &= ~LED;	// Turn off LED
		while((GPIO_PORTF_DATA_R & (SW1 | SW2))!= (SW1|SW2)){	// When either switch is pressed
				if(i<50){
				now = NVIC_ST_CURRENT_R;
				Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
				Data[i] = GPIO_PORTF_DATA_R&(LED | SW1 | SW2); // record PF0,1,4
				last = now;
				i++;
			}			
			GPIO_PORTF_DATA_R ^= LED;		// Toggle LED
			Delay();
		}
